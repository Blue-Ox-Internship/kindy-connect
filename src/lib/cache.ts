 ...
 *
 * This module runs ONLY on the server (Node.js process) and is never shipped
 * to the browser. It is used internally by db-functions.ts for caching
...
class ServerCacheManager {
  ...
  /**
   * Get current cache stats (server-side only, not exposed to clients)
   */
  getStats(): CacheStats {
